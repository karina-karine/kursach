...
function setEqualHeights(selector) {
    requestAnimationFrame(() => {
        ...
        if (cards.length === 0) {
            console.log(`setEqualHeights: No cards found for selector "${selector}".`);
            return;
        }
        let maxHeight = 0;
        ...
        console.log(`setEqualHeights: Applied max height of ${maxHeight}px to ${selector} cards.`);
    });
}
...
function observeServiceCards() {
    ...
    if (targets.length === 0) {
        console.warn("MutationObserver: No target containers (.services-track, #all-services-grid, or .reviews-track) found for observation.");
        return;
    }
    ...
    const observerCallback = debounce((mutationsList, observer) => {
        ...
        if (shouldRecalculate) {
            console.log('MutationObserver: Detected changes in service/review cards container, recalculating heights.');
    ...
    targets.forEach(target => {
        ...
        console.log(`MutationObserver: Started observing ${target.id || target.className} for height adjustments.`);
    });
}
...
function setupFormLogic(formElement) {
    ...
    if (uniquenessSlider && uniquenessValue) {
        ...
        uniquenessSlider.addEventListener("input", () => {
            uniquenessValue.textContent = `${uniquenessSlider.value}%`;
            console.log(`Slider value changed for ${formElement.id}:`, uniquenessSlider.value);
        });
    } else {
        console.warn(`Uniqueness slider or value element not found within form: ${formElement.id}`);
    }
    ...
    if (fileInput && fileLabel) {
        ...
        console.warn(`File input or label not found within form: ${formElement.id}`);
    }

    // Form Submission (if it's an order form)
    if (formElement.id === "orderForm" || formElement.id === "modalOrderForm") {
        const formMessageElement = formElement.querySelector("#form-message") || formElement.querySelector("#modal-form-message");
        ...
        formElement.addEventListener("submit", async (event) => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    formElement.reset();
                    if (fileLabel) { // Reset file label after successful submission
                        fileLabel.innerHTML = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                                                    <path d="M10 5v10M5 10h10" stroke="currentColor" stroke-width="2" />
                                                </svg> Додати файли`;
                    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (orderFormModal) {
        orderFormModal.addEventListener("click", (event) => {
            if (event.target === orderFormModal) { // Перевіряємо, чи клік був саме по оверлею, а не по формі
                closeOrderModal();
            }
        });
    }
